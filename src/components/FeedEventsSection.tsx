...
import { EventInterestsFilterToggle } from "@/components/EventInterestsFilterToggle"
import { EventSearchToggle } from "@/components/EventSearchToggle"
import {
  eventMatchesSelectedInterests,
  getPrimaryEventInterestLabel,
...
export function FeedEventsSection({ events }: FeedEventsSectionProps) {
  ...
  const filteredEvents = useMemo(() => {
    ...
    const hasInterestFilter = selectedInterestIds.length > 0

    if (!hasSearchFilter && !hasDateFilter && !hasInterestFilter) {
      return typedEvents
    }
    ...
    return typedEvents.filter((event) => {
      ...
      const matchesDate = hasDateFilter
        ...
        : true
      const matchesInterests = eventMatchesSelectedInterests(
        event,
        selectedInterestIds,
      )
      return matchesSearch && matchesDate && matchesInterests
    })
  }, [typedEvents, query, selectedDateLabels, selectedInterestIds])
  ...
  const isInterestFilterActive = selectedInterestIds.length > 0
  const hasNoResults =
    (isSearchActive || isDateFilterActive || isInterestFilterActive) &&
      ...
      <header className="px-6">
        <div className="mt-4 flex flex-wrap items-center gap-2 px-4">
          ...
          <div className="ml-auto flex flex-wrap items-center justify-end gap-2">
            <EventDateFilterToggle
              value={selectedDates}
              onChange={setSelectedDates}
            />
            <EventDateSortToggle
              descending={dateSortDescending}
              onChange={setDateSortDescending}
            />
      ...
      {hasNoResults ? (
        <div className="mt-3 px-6">
          <div className="rounded-3xl border border-sky-200/55 bg-[linear-gradient(150deg,oklch(1_0_0_/_0.93),oklch(0.972_0.022_238_/_0.78)_50%,oklch(0.942_0.045_215_/_0.6))] px-4 py-5 text-sm text-muted-foreground shadow-[0_26px_54px_-38px_rgba(56,98,141,0.34)] backdrop-blur-xl dark:border-white/12 dark:bg-[linear-gradient(150deg,oklch(0.22_0.02_250_/_0.66),oklch(0.17_0.02_252_/_0.56)_52%,oklch(0.13_0.02_250_/_0.42))]">
            ...
            <p className="mt-1">
              Adjust search text, selected dates, or interests.
            </p>
          </div>
        </div>
      ) : (
            ...
            <li key={event._id} className="px-6">
              {(() => {
                ...
                const cardSizeClass = hasPreview
                  ...
                  : "min-h-[220px]"
                const interestLabel =
                  getPrimaryEventInterestLabel(event) ??
                  ...
                  >
                    ...
                    <div>
                        ...
                        <div className="flex min-h-9 min-w-0 flex-1 items-center">
                          <div className="flex flex-wrap items-baseline gap-2 text-mono-label">
                            <span
                              className={
                                categoryColor[
                                  event.category as keyof typeof categoryColor
                                ]
                              }
                            >
                              {event.day ?? ""}
                      ...
                      <div className="mt-2 pl-10">
                        ...
                        <div
                          className={`mt-4 flex items-center gap-4 ${hasPreview ? "" : "pt-2"}`}
                        >
                          {hasPreview ? (
                            <div
