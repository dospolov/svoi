...
import { events, getEvent, categoryColor, gradientClass } from "@/lib/events";
...
export default async function EventPage({ params }: PageProps) {
  ...
  if (!event) {
    ...
    const more = events.filter((e) => e.id !== id).slice(0, 3);
    ...
    return (
      <div className="mx-auto min-h-screen w-full max-w-[430px] bg-background pb-32 text-foreground">
        ...
        <section className="mt-6 px-6">
          <div
            className={`relative overflow-hidden rounded-3xl ${gradientClass[event.gradient]} aspect-[4/5]`}
          >
            ...
            <div className="absolute left-4 right-4 top-4 flex items-start justify-between">
              <div className="flex items-center gap-2 rounded-full bg-black/50 px-3 py-1.5 backdrop-blur-md">
                ...
                <span className="text-xs font-medium text-foreground">
                  Featured · {event.category[0].toUpperCase() + event.category.slice(1)}
                </span>
              </div>
              <button
                type="button"
            ...
            <div className="absolute inset-x-0 bottom-0 p-5">
              ...
              <div className="mt-4 flex items-end justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-white/90">{event.going.split("/")[0]} going</span>
                </div>
                <div className="text-right text-sm text-white/80">
                  <MapPin className="mr-1 inline h-3.5 w-3.5" />
                  {event.neighborhood} · {event.distance.toLowerCase()}
                </div>
              </div>
          ...
          <ul className="mt-4 space-y-3">
            {more.map((e) => (
              <li key={e.id}>
                ...
                >
                  <div
                    className={`flex h-20 w-20 shrink-0 items-end rounded-xl p-2 ${gradientClass[e.gradient]}`}
                  >
                    <span className="rounded-md bg-black/50 px-2 py-0.5 text-[11px] font-medium text-white backdrop-blur">
                      {e.day[0] + e.day.slice(1, 3).toLowerCase()}
                    </span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className={`text-mono-label ${categoryColor[e.category]}`}>
